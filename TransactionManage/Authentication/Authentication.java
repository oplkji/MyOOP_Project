package TransactionManage.Authentication;
...
public class Authentication extends JFrame {
    ...
    public Authentication(JFrame frame) {
        JLabel announce = new JLabel("Xác thực mã PIN");
        ...
        this.setTitle("PIN Authentication");
    ...
    public static boolean checkPIN(){
        String sql = "SELECT PIN FROM signup WHERE userID = ?";
        try(Connection conn = ConnectDB.getConnection();
            PreparedStatement stmt = conn.prepareStatement(sql);){
            stmt.setString(1, BankManagement.getUserID());
            ResultSet rs = stmt.executeQuery();
            if(rs.next()){
                return new String(passwordField.getPassword()).equals(rs.getString("PIN"));
            }
